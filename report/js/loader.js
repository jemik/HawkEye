...
function drawProcessFlow(data) {
	...
	var skewX = -29; // skew angle in degrees
	var scaleY = 1.0 // scale factor (0.0 - 1.0)
	var skewXDegrees = -29; // Adjusted skew angle in degrees
	...
	svgGroup.selectAll("g.node").each(function () {
        ...
        // If the scaleY is less than 1, the graph is compressed vertically
        var scaleOffset = (1 - scaleY) * (nodeData.y - svgHeight / 4);

        // Adjust the node's position by the skew offset
        var newX = nodeData.x + skewOffset;
        var newY = nodeData.y - scaleOffset;
	...
	svgGroup.selectAll('.edgePath path')
		.attr('marker-end', 'url(#standard-arrowhead1)');
